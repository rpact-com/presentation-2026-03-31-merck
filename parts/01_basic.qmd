...
next_best <- NextBestNCRMLoss(
    target = c(0.2, 0.3), # loss: 0
    overdose = c(0.4, 0.7), # loss: 2
    unacceptable = c(0.7, 1), # loss: 10
    max_overdose_prob = 0.25,
    losses = c(1, 0, 2, 10) # underdosing has loss 1
)
next_best
```
...
loss_df <- next_best_dose$probs |> as.data.frame() |> dplyr::select(dose, posterior_loss)
result_df <- dplyr::left_join(fitted_df, loss_df, by = "dose") 
knitr::kable(result_df, digits = 3)
```
