...
- The package's repo is hosted on GitHub at [github.com/openpharma/crmPack](https://github.com/openpharma/crmPack)
...
1. Implement a constructor / initialization function for the new class, which specifies the likelihood and prior distributions in the underlying `GeneralModel` slots:
   1. The `datamodel` slot should contain the JAGS code for the likelihood
   1. The `priormodel` slot should contain the JAGS code for the prior distributions
   1. The `modelspecs` slot is a function that returns required JAGS inputs
   1. The `init` slot is a function that returns initial values for the MCMC sampling
   1. The `datanames` slot is a character vector of the names of the data inputs required for the likelihood
1. Implement `dose()` and `prob()` methods for the new class, which compute the dose and toxicity probability for given model parameters using `setMethod()`

Full examples are available [in the JSS paper](https://www.crmpack.org/vignettes/getting-started/crmPack_paper/#extending-crmpack-functionality) and [in the parallelization vignette](https://www.crmpack.org/vignettes/utilities/crmPack_parallel_computing/#worked-out-example).
